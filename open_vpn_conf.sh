#!/bin/bash
# ===============================================================
# open_vpn_conf.sh - Gerenciador OpenVPN (Fix Argumento Interactive)
# ===============================================================

DIR_SCRIPTS="/opt/configdebian"

# --- DETECÇÃO DO USUÁRIO REAL ---
USER_ATUAL=$(stat -c '%U' $(tty) 2>/dev/null)
if [ "$USER_ATUAL" == "root" ] || [ -z "$USER_ATUAL" ]; then
    USER_ATUAL=$(who am i | awk '{print $1}')
fi
[ -z "$USER_ATUAL" ] && USER_ATUAL="$SUDO_USER"
...
menu_ovp() {
    while true; do
        ...
        echo -e "${AZUL}===============================================================${NC}"
        echo -e "  [1] 👤 Criar / Remover Usuários (Interactive)"
        ...
        case $OPCAO in
            1) 
                if [ -f "$INSTALLER_PATH" ]; then
                    # CHAMADA CORRIGIDA COM ARGUMENTO INTERACTIVE
                    bash "$INSTALLER_PATH" interactive
                    organizar_arquivos
                else
                    echo -e "${VERMELHO}Erro: $INSTALLER_PATH não encontrado!${NC}"
                    sleep 2
                fi
                ;;
